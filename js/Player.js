...
class QB extends Player {
    ...
    update() {
        ...
        if (this.getData("speed") < this.getData("speedOriginal")) {
            this.setData("speed", this.getData("speed") + 0.1);
        }
        if (newGame) {
            if (!this.getData("moving")) {
                this.play(player1Key);
            ...
            if (this.x < this.getData("startX")) {
                this.x += this.getData("speed");
            }
            if (this.x > this.getData("startX")) {
                this.x -= this.getData("speed");
            }
            if (this.y < this.getData("startY")) {
                this.y += this.getData("speed");
            }
        }
        if (ballSnapped) {
            if (!this.getData("moving") && !this.getData("spinning") && !this.getData("blasting")) {
                this.play(player1Key);
                this.setData("moving", true);
            }
            if (offensiveTeam == 1) {
    ...
    update() {
        ...
        if (ballSnapped) {
            ...
            if (this.getData("chasing")) {
                if (this.x < football.x) {
                    this.x += this.getData("speed");
                }
                if (this.x > football.x) {
                    this.x -= this.getData("speed");
                }
                if (this.y < football.y) {
                    this.y += this.getData("speed");
                    this.scaleY = -1;
                }
                if (this.y > football.y) {
            ...
            this.scaleY = 1;
        } //end ballSnapped//end ballSnapped
    }
}
    ...
    update() {
        ...
        if (difficulty == 2.4) { //LB following play Hall of Fame mode
            if (!inPlay && defensiveTeam == 2 && team2Formation != "kickOff") {
                ...
                if (passPlay == true) {
                    if (this.y > this.getData("startY") - 60) {
                        this.y -= this.getData("speed");
                    }
                }
            }
